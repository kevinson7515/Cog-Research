...
# Plan Creation Rules
1. Create a small number of high-level steps (3-10 steps is ideal)
...
3. 维护清晰的步骤依赖关系，并按有向无环图结构组织计划
4. 仅在无现有计划时创建新计划；否则更新现有计划
5. 计划的最后一步，必须是生成一份多模态的最终报告，最终报告依赖所有前置步骤
...
# 示例
计划创建示例：
对于任务“帮我写一篇中兴通讯的分析报告”，计划可能为：
标题：撰写中兴通讯分析报告
步骤：["收集中兴通讯基础信息与最新公开资料", "分析行业环境与竞争格局", "分析公司业务结构、财务表现与核心竞争力", "梳理风险因素与未来展望", "生成最终多模态报告"]
依赖项：{"1": [0], "2": [0, 1], "3": [2], "4": [0, 1, 2, 3]}
"""
    else:
        # 原始完整版本
        system_prompt = """
...
5. The final step in the plan must be to generate a multimodal final report, which depends on all the preceding steps.
...
For a task "Help me write an analysis report on ZTE Corporation"", the plan could be:
title: Writing an Analysis Report on ZTE Corporation
steps: ["Collect basic information and the latest publicly available data on ZTE Corporation", "Analyze the industry environment and competitive landscape", "Analyze the company's business structure, financial performance, and core competitiveness", "Analyze risk factors and future prospects", "Generate the final multimodal report"]
dependencies: {"1": [0], "2": [0, 1], "3": [2], "4": [0, 1, 2, 3]}
"""
    return system_prompt
...
"""
    # 根据模型类型提供不同的规划指导
        ...
        create_plan_prompt = f"""
创建一个包含 3-10 个步骤的简洁且聚焦的计划以完成此任务：{question}
请记住保持步骤简洁，并仅包含真正必要的内容。
"""
    elif is_claude and not contains_chinese:
        create_plan_prompt = f"""
Create a simple, focused plan with 3-10 steps to accomplish this task: {question}
Remember to keep steps concise and only include what's truly necessary.
"""
    elif not is_claude and contains_chinese:
        create_plan_prompt = f"""
使用 create_plan 工具，创建一个包含 3-10 个步骤的简洁且聚焦的计划以完成此任务：{question}
请记住保持步骤简洁，并仅包含真正必要的内容。
"""
    else:
        create_plan_prompt = f"""
Using the create_plan tool, create a detailed plan of 3-10 steps to accomplish this task: {question},
