...
def serper_search_skill():
    return {
        'skill_name': 'serper_search',
        'skill_type': "function",
        'display_name_zh': 'Serper搜索',
        'display_name_en': 'Serper Search',
        'description_zh': '使用Serper搜索引擎搜索给定查询的信息',
        'description_en': 'Use Serper search engine to search information for the given query',
        'semantic_apis': ["api_search"],
        'function': SkillFunction(
            id='3c44f9ad-be5c-4e6c-a9d8-1426b23828a0',
            name='app.cosight.search_toolkit.search_google',
            description_zh='通过Serper搜索引擎获取查询结果',
            description_en='Get search results using Serper search engine',
            parameters={
                "type": "object",
                "properties": {
                    "query": {
...
def register_mcp_tools():
    # 解析mcp工具
    mcp_config_path = os.path.join(mcp_server_config_dir, "mcp_server_config.json")
    configured_skills = JsonUtil.read_data(mcp_config_path)
    if isinstance(configured_skills, dict):
        configured_skills = [configured_skills]
    if not isinstance(configured_skills, list):
        return []

    required_fields = {"skill_name", "skill_type", "display_name_zh", "display_name_en"}
    return [
        skill
        for skill in configured_skills
        if isinstance(skill, dict) and required_fields.issubset(skill)
    ]
...
def generate_markdown_report_skill():
    """Markdown report generation skill for the Agent framework."""
    return {
        'skill_name': 'generate_markdown_report',
        'skill_type': "function",
        'display_name_zh': 'Markdown报告生成',
        'display_name_en': 'Generate Markdown Report',
        'description_zh': '根据工作区中的文本文件生成结构化Markdown报告，自动汇总真实网页参考文献并嵌入被引用的本地图片。仅在需要生成最终报告时调用。',
        'description_en': 'Generate a structured Markdown report from workspace text files, automatically compiling real web references and embedding referenced local images. Use this only when creating the final report.',
        'semantic_apis': ["api_report_generation"],
        'function': SkillFunction(
            id='0d9c795c-8f5c-4e74-b82f-0d9ff89ff2fb',
            name='app.cosight.tool.report_toolkit.main',
            description_zh='基于工作区中的文本文件自动生成Markdown格式的最终报告，保留真实URL引用和本地图片相对路径。',
            description_en='Automatically generate a final Markdown report based on workspace text files while preserving real URL citations and local image relative paths.',
            parameters={
                "type": "object",
                "properties": {
                    "title": {
                        "type": "string",
                        "description_zh": "可选的报告标题。不提供时会自动生成。",
                        "description_en": "Optional report title. If omitted, the title is generated automatically."
                    },
                    "output_filename": {
                        "type": "string",
                        "description_zh": "可选的输出文件名前缀，不包含扩展名。",
                        "description_en": "Optional output filename prefix without extension."
                    },
                    "user_query": {
                        "type": "string",
                        "description_zh": "原始用户问题/任务，用于匹配报告语言和写作范围。",
                        "description_en": "Original user question/task, used to match report language and scope."
                    },
                    "include_sources": {
                        "type": "boolean",
                        "description_zh": "是否在报告末尾附加真实网页参考文献列表，默认True；不会附加本地中间文件列表。",
                        "description_en": "Whether to append the real web reference list to the report. Defaults to true; local intermediate file lists are not appended.",
                        "default": True
                    }
                },
                "required": []
