...
def strip_internal_compression_markers(content: str) -> str:
    """Remove agent-internal tool-call compression notes before saving user files."""
...
class FileToolkit:
    ...
    def file_read(self, file: str, start_line: int = None, end_line: int = None, sudo: bool = False,
        ...
        r"""Read file content. Supports text files and common document files.

        For PDF, Word, and Excel files, this method extracts readable text
        unless binary=True is explicitly requested.
        ...
        try:
            ...
            # 在Read file之前添加的健壮性代码，自动检测文件类型，如果是文本就是utf-8读取，如果是图片就是binary读取
            extension = Path(absolute_path).suffix.lower()
            document_extensions = {".doc", ".docx", ".pdf", ".xls", ".xlsx"}
            if extension in document_extensions and not binary:
                from app.cosight.tool.document_processing_toolkit import DocumentProcessingToolkit

                return DocumentProcessingToolkit().extract_document_content(absolute_path)

            import mimetypes
            mime, _ = mimetypes.guess_type(absolute_path)
            is_binary = binary or (mime is None or not mime.startswith("text"))
            ...
            if is_binary and not binary:
                return (
                    ...
                    "Use the dedicated image/video/audio analysis tool for media files, "
                    "or call file_read with binary=True only when raw bytes are required."
                )
