...
class SearchToolkit:
    ...
    def serper_search(
        ...
        r"""Use Serper Search API to search information for the given query.
        Args:
            query (str): The query to be searched.
            num_results (int): The number of search results to retrieve
                (default is `5`).
            **kwargs: Extra Serper request parameters, e.g. `gl`, `hl`, `tbs`,
                `autocorrect`, etc. They will be forwarded into the request body.
        ...
        if not serper_api_key:
            raise ValueError(
                ...
                "Get `SERPER_API_KEY` from Serper and set it in your env."
            )
        ...
        # Allow caller to pass additional Serper parameters
        ...
        try:
            ...
            # 如果没有 organic 结果，但有 knowledgeGraph，可以补一个兜底结果
            if not results and data.get("knowledgeGraph"):
                kg = data["knowledgeGraph"]
                results.append({
                    "result_id": 1,
                    "title": kg.get("title", query),
                    "description": kg.get("description", ""),
                    "long_description": kg.get("description", ""),
                    "url": kg.get("website", ""),
            ...
            logger.info(f"serper_search result = {data}")
            return results

        except requests.exceptions.HTTPError as e:
            logger.error(
                f"Serper HTTP error: {e}, response text: {getattr(response, 'text', '')}",
                exc_info=True,
            )
            return [{
                "error": f"Serper HTTP error: {e}",
