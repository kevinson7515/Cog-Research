...
class ToolResultProcessor:
    ...
    def process_tool_result(tool_name: str, tool_args: str, tool_result: str, task_title: str = "") -> Dict[str, Any]:
        ...
        try:
            # 根据工具名称精确匹配选择处理方式
            # if tool_name in ['search_baidu', 'search_google', 'search_wiki', 'tavily_search', 'image_search', 'serper_search']:
            #     return ToolResultProcessor._process_search_result(tool_name, tool_args, tool_result, task_title)
            if tool_name in ['serper_search', 'image_search']:
                return ToolResultProcessor._process_search_result(tool_name, tool_args, tool_result, task_title)
    ...
    def _process_search_result(tool_name: str, tool_args: str, tool_result: str, task_title: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            # Iframe embeddability is only a UI display concern. A page that
            # blocks iframe embedding can still be fetched and scraped, so do
            # not discard search URLs based on X-Frame-Options/CSP.
            ...
            return {
                ...
                "summary": ToolResultProcessor._get_localized_summary(
                    f"搜索完成，找到 {result_count} 个结果；iframe可嵌入性不作为抓取过滤条件",
                    f"Search completed, found {result_count} results; iframe embeddability is not used as a scraping filter",
                    task_title
                ),
                "first_url": first_url,
