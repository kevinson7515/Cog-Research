...
class ChatLLM:
    ...
    def _is_dashscope_qwen(self) -> bool:
        """判断是否为 DashScope Qwen 兼容模式接口。

        DashScope 的 Qwen 在非流式调用时要求显式关闭 thinking，否则会报错：
        `parameter.enable_thinking must be set to false for non-streaming calls`。
        """
        base_url = (self.base_url or "").lower()
        model = (self.model or "").lower()
        return ("aliyuncs.com" in base_url) and ("qwen" in model)

    def _ensure_non_streaming_thinking_disabled(self, api_params: Dict[str, Any]) -> None:
        """对 DashScope Qwen 非流式请求强制关闭 thinking。"""
        if not self._is_dashscope_qwen():
            return
    ...
    def _sanitize_messages_for_gemini(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Gemini 兼容清洗：
        1. 合并所有 system 消息为一条
        2. 合并连续的 user/user 或 assistant/assistant
        3. 尽量避免出现不必要的空消息
        """
        ...
        for msg in messages:
            ...
            # 连续同角色消息合并
            if sanitized_messages and sanitized_messages[-1].get("role") == role and role in ("user", "assistant"):
                prev = sanitized_messages[-1]

                # assistant 且带 tool_calls 时，不建议硬合并
                if role == "assistant" and (prev.get("tool_calls") or msg_copy.get("tool_calls")):
                ...
                # 尽量保留 reasoning_content
                if "reasoning_content" in msg_copy and "reasoning_content" not in prev:
                    prev["reasoning_content"] = msg_copy["reasoning_content"]
                continue
        ...
        # 如果最前面不是 system/user，删掉前面的孤立消息，避免 Gemini 更严格的校验报错
        while sanitized_messages and sanitized_messages[0].get("role") not in ("system", "user"):
            logger.warning(f"Dropping leading {sanitized_messages[0].get('role')} message for Gemini compatibility")
            sanitized_messages.pop(0)

        return sanitized_messages

    def _sanitize_tools_for_gemini(self, tools: List[Dict]) -> List[Dict]:
        """
        Gemini 工具 Schema 强效清洗：
        1. 提取标准的 type 和 function 结构
        2. 将 description_zh/en 转换为标准的 description
        3. 移除不支持的 additionalProperties
        """
        if not tools:
            return tools
        ...
        for tool in tools:
            # 1. 结构提取：只保留 OpenAI 标准的 type 和 function 字段
            # 兼容你代码中可能直接传整个技能字典的情况
            standard_tool = {}
            if "type" in tool and "function" in tool:
                standard_tool = copy.deepcopy(tool)
            elif "skill_type" in tool and "function" in tool:
                standard_tool = {
                    "type": "function",
                    "function": copy.deepcopy(tool["function"])
                }
            else:
                continue # 跳过不符合基本结构的工具

            func = standard_tool["function"]
            # 处理 SkillFunction 对象转为字典（如果是对象的话）
            if hasattr(func, 'to_dict'):
                func = func.to_dict()
                standard_tool["function"] = func
            ...
            if "parameters" in func:
                ...
                def recursive_clean(obj):
                    ...
                    # 将自定义的 description_zh 合并到标准 description
                    if "description_zh" in obj:
                        if "description" not in obj:
                            obj["description"] = obj["description_zh"]
                        del obj["description_zh"]
                    if "description_en" in obj:
                        del obj["description_en"]
                    
                    # 移除 Gemini 容易报错的 additionalProperties
                    ...
                    if "properties" in obj:
                        for p in obj["properties"].values():
                            recursive_clean(p)
                    if "items" in obj:
                        recursive_clean(obj["items"])

                recursive_clean(params)
            
            sanitized_tools.append(standard_tool)
    ...
    def _count_tokens(self, messages: List[Dict[str, Any]]) -> int:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"tiktoken encoding unavailable, using simplified token estimation: {e}")
                    ...
                    if isinstance(value, str):
                        ...
                        for tool_call in value:
                            if isinstance(tool_call, dict):
                                ...
                                total_tokens += len(encoding.encode(str(function_obj.get("name", ""))))
                                total_tokens += len(encoding.encode(str(function_obj.get("arguments", ""))))
            ...
            logger.debug(f"Token counting failed, using simplified token estimation: {e}")
            return self._estimate_tokens_simple(messages)
...
Keep facts, data, file paths. Remove redundancy. Output summary only:"""
    ...
    def create_with_tools(
        ...
        for attempt in range(max_retries):
            try:
                # 由于 gemini 的接口规范比较严格，这里做一下 gemini 专用的 messages 清洗 和 tools 清洗
                if self._is_gemini_model():
                    messages = self._sanitize_messages_for_gemini(messages)
                    if tools:
                        tools = self._sanitize_tools_for_gemini(tools)
                ...
                # DashScope Qwen 兼容模式：非流式调用必须显式关闭 thinking
                self._ensure_non_streaming_thinking_disabled(api_params)
        ...
        if response and isinstance(response, ChatCompletion):
            # 去除think标签，并保留 finish_reason 供上层检测输出截断。
            choice = response.choices[0]
            message = choice.message
            content = message.content
            if content:
                preview = content[:1000] + ("...[truncated]" if len(content) > 1000 else "")
                logger.debug(f"RAW LLM OUTPUT preview: >>>{preview}<<<")
            try:
                object.__setattr__(message, "finish_reason", getattr(choice, "finish_reason", None))
                object.__setattr__(message, "completion_tokens", getattr(getattr(response, "usage", None), "completion_tokens", None))
            except Exception:
                pass
            if content is not None and '</think>' in content:
                message.content = content.split('</think>')[-1].strip('\n')
            return message
        else:
            raise ZaeFrameworkException(
                400,
                "chat with LLM failed: empty or invalid response from provider. "
                f"model={self.model}, base_url={self.base_url}, "
                f"message_count={len(messages)}, tool_count={len(tools)}, "
                f"max_tokens={self.max_tokens}, response={response}"
            )
        ...
        if response.choices[0].message.tool_calls:
            for attempt in range(3):
                try:
                    ...
                    # 本地快速修复：将 {1: [0]} 这种“数字 key”改成合法 JSON 的 {"1": [0]}
                    try:
                        candidate = tool_call.arguments
                        if isinstance(candidate, str) and "{" in candidate and ":" in candidate:
                            candidate = re.sub(r'([{,]\s*)(\d+)\s*:', r'\1"\2":', candidate)
                            json.loads(candidate)
                            tool_call.arguments = candidate
                            logger.info("Successfully fixed tool call arguments locally (numeric keys).")
                            break
                    except Exception:
                        pass

                    try:
                        # 优化提示词：严禁输出 Markdown 标记
                        fixed_arguments = self.chat_to_llm([{"role": "user",
                                                           "content": f"下面的json字符串格式有错误，请帮忙修正。重要：仅输出纯净的JSON字符串，绝对不要包含任何markdown格式标记（如```json）。\n{tool_call.arguments}"}])
                        
                        # 新增：清洗 LLM 返回的字符串，剥离可能存在的 Markdown 栅栏
                        cleaned_args = fixed_arguments.strip()
    ...
    def chat_to_llm(self, messages: List[Dict[str, Any]], max_tokens: int = None):
        ...
        for attempt in range(max_retries):
            try:
                # 每次重试时，使用当前循环的 messages，避免 Gemini 的清洗逻辑破坏原始结构
                current_messages = messages
                
                # 由于 gemini 的接口规范比较严格，这里做一下 gemini 专用的 messages 清洗
                if self._is_gemini_model():
                    current_messages = self._sanitize_messages_for_gemini(current_messages)
                ...
                if not (hasattr(response, 'choices') and response.choices and len(response.choices) > 0):
                    raise Exception(f"LLM returned no choices: {response}")
                response_content = response.choices[0].message.content
                if response_content is None or not str(response_content).strip():
                    raise Exception(f"LLM returned empty content: {response}")
                break  # 成功调用，跳出重试循环

            except Exception as e:
                error_str = str(e)
                # 检查是否是上下文长度超限错误（包含本次日志中遇到的 "too large" 关键词）
                if "maximum context length" in error_str.lower() or "context length" in error_str.lower() or "too large" in error_str.lower():
                    logger.warning(
                        f"Context length exceeded on attempt {attempt + 1} in chat_to_llm. "
                        f"Truncating messages more aggressively..."
                    )
                    # 更激进地截断消息：减少保留的消息数量
                    original_max = self.max_messages
                ...
                if attempt == max_retries - 1:
                    logger.error(f"Failed to create after {max_retries} attempts in chat_to_llm.")
                    raise ZaeFrameworkException(400, f"chat with LLM failed, please check LLM config. reason：{e}")
                time.sleep(3)  # 增加等待时间，避免频繁重试
