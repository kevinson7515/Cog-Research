...
def _compact_plan_text(text: str, max_chars: int = None) -> str:
    text = str(text or "")
    if max_chars is None:
        try:
            max_chars = _env_int("STEP_NOTE_MAX_CHARS", 4096)
        except Exception:
            max_chars = 4096
    if max_chars <= 0 or len(text) <= max_chars:
        if text.startswith("[Parsed XML tool_call text and moved it to structured tool_calls:"):
            return ""
        return text
    ...
    if "<tool_call>" in text and "</tool_call>" not in text:
        return (
            f"[Truncated incomplete tool_call omitted from plan notes: original {len(text)} chars. "
            "The agent should retry with compressed tool arguments.]"
        )

    important_patterns = (
        "http://", "https://", "source", "来源", "引用", "title", "标题",
        "date", "日期", "path", "文件", "saved", "保存", "结论", "conclusion",
        "error", "错误", "blocked", "completed",
    )
    selected = []
    seen = set()
    for line in text.splitlines():
        ...
        if sum(len(item) + 1 for item in selected) >= int(max_chars * 0.8):
            break

    compacted = "\n".join(selected).strip()
    if len(compacted) < min(500, max_chars):
        compacted = text[:max_chars].rstrip()
    if len(compacted) > max_chars:
        compacted = compacted[:max_chars].rstrip()
    return f"{compacted}\n\n[Plan note compacted: original {len(text)} chars, kept <= {max_chars} chars.]"
...
class Plan:
    ...
    def __init__(self, title: str = "", steps: List[str] = None, dependencies: Dict[int, List[int]] = None, work_space_path: str = ""):
        ...
        # Only a successfully validated generate_markdown_report call may set
        # this path. Planner/actor prose must never be treated as an artifact.
    ...
    def set_final_report_path(self, report_path: str) -> None:
        with self._lock:
            self.final_report_path = str(report_path or "")

    def get_final_report_path(self) -> str:
        with self._lock:
            return self.final_report_path

    def fail_step_artifact_contract(self, step_index: int, step_notes: str) -> None:
        """Block a step whose claimed output failed final artifact validation."""

        with self._lock:
            if step_index < 0 or step_index >= len(self.steps):
                raise ValueError(
                    f"Invalid step_index: {step_index}. Valid indices range from 0 to {len(self.steps) - 1}."
                )
            step = self.steps[step_index]
            self.step_statuses[step] = "blocked"
            compacted = _compact_plan_text(step_notes)
            compacted, file_path_info = process_text_with_workspace(compacted, self.work_space_path)
            self.step_notes[step] = compacted
    ...
    def get_ready_steps(self) -> List[int]:
        ...
        with self._lock:
            ...
            # 检查所有依赖是否都已完成
                if self._dependencies_completed(step_index, dependencies):
                # 检查步骤本身是否未开始
    ...
    def claim_step(self, step_index: int) -> bool:
        """Atomically reserve a ready step for execution."""
        with self._lock:
            if step_index < 0 or step_index >= len(self.steps):
                raise ValueError(f"Invalid step_index: {step_index}. Valid indices range from 0 to {len(self.steps) - 1}.")
            step = self.steps[step_index]
            if self.step_statuses.get(step) != "not_started":
                return False
            if not self._dependencies_completed(step_index):
                return False
            self.step_statuses[step] = "in_progress"
            return True
    ...
    def mark_step(self, step_index: int, step_status: Optional[str] = None, step_notes: Optional[str] = None) -> None:
        ...
        if step_index < 0 or step_index >= len(self.steps):
            raise ValueError(f"Invalid step_index: {step_index}. Valid indices range from 0 to {len(self.steps) - 1}.")
        note_len = len(str(step_notes)) if step_notes is not None else 0
        logger.info(f"step_index: {step_index}, step_status is {step_status}, step_notes_len={note_len}")
        step = self.steps[step_index]
        current_status = self.step_statuses.get(step)

        if current_status == "completed" and step_status in ("in_progress", "blocked"):
            logger.warning(
                "Ignoring status regression for completed step %s: attempted %s",
                step_index,
                step_status,
            )
            return
        ...
        if step_status in ("in_progress", "completed"):
            # Check if all dependencies are completed
            if not self._dependencies_completed(step_index):
                raise ValueError(f"Cannot {step_status.replace('_', ' ')} step {step_index} before its dependencies are completed")

        # Update step status after dependency validation so failed marks do not
        ...
        if step_notes is not None:
            step_notes = _compact_plan_text(step_notes)
            step_notes, file_path_info = process_text_with_workspace(step_notes, self.work_space_path)
            self.step_notes[step] = step_notes
            self.step_files[step] = file_path_info

    def _dependencies_completed(self, step_index: int, dependencies: Optional[List[int]] = None) -> bool:
        dependencies = self.dependencies.get(step_index, []) if dependencies is None else dependencies
        for dep in dependencies:
            dep_index = int(dep)
            if dep_index < 0 or dep_index >= len(self.steps):
                return False
            if self.step_statuses.get(self.steps[dep_index]) != "completed":
                return False
        return True
    ...
    def add_tool_call(self, step_index: int, tool_name: str, tool_args: str, tool_result: str = None) -> None:
        ...
        if step_index == -1:
            ...
            tool_call_info = {
                "tool_name": tool_name,
                "tool_args": _compact_plan_text(tool_args, _env_int("TOOL_CALL_RECORD_MAX_CHARS", 4096)),
                "tool_result": _compact_plan_text(tool_result, _env_int("TOOL_RESULT_RECORD_MAX_CHARS", 2048)) if tool_result is not None else None,
    ...
    def format_for_llm(
        ...
        *,
        ...
        max_note_chars: int = 4096,
    ) -> str:
        """
        A compact plan formatter designed for LLM prompts.

        Differences from `format()`:
        - Always omits verbose step notes/details by default (they can be huge and explode context).
        - Optionally includes *truncated* notes only for the current step and its dependencies.
        """
        output = f"Plan: {self.title}\n"
        output += "=" * len(output) + "\n\n"
        ...
        output += (
            f"Status: {progress['completed']} completed, {progress['in_progress']} in progress, "
            f"{progress['blocked']} blocked, {progress['not_started']} not started\n\n"
        )
        output += "Steps:\n"

        def _status_symbol(step_name: str) -> str:
            return {
                "not_started": "[ ]",
                "in_progress": "[>]",
                "completed": "[✓]",
                "blocked": "[!]",
            }.get(self.step_statuses.get(step_name), "[ ]")

        def _truncate_note(note: str) -> str:
            note = (note or "").strip()
            if not note:
                return ""
            if max_note_chars is not None and len(note) > max_note_chars:
                return note[: max_note_chars].rstrip() + " …(truncated)"
            return note

        # Decide which steps' notes to include (only relevant ones)
        notes_step_indexes = set()
        if include_notes and current_step_index is not None:
            stack = [int(current_step_index)]
            while stack:
                idx = stack.pop()
                if idx in notes_step_indexes or idx < 0 or idx >= len(self.steps):
                    continue
                notes_step_indexes.add(idx)
                for dep in self.dependencies.get(idx, []) or []:
                    try:
        ...
        for i, step in enumerate(self.steps):
            deps = self.dependencies.get(i, [])
            dep_str = f" (depends on: {', '.join(map(str, deps))})" if deps else ""
            output += f"Step{i}: {_status_symbol(step)} {step}{dep_str}\n"
            if include_notes and i in notes_step_indexes:
                note = _truncate_note(self.step_notes.get(step, ""))
                if note:
                    output += f"  Notes: {note}\n"
