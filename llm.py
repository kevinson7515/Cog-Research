...
def _redact_model_config(model_config: dict) -> dict:
...
def set_model(model_config: dict[str, Optional[str | int | float]]):
    ...
    if model_config.get('proxy') is not None:
        chat_llm_kwargs['proxy'] = model_config['proxy']

    return ChatLLM(**chat_llm_kwargs)


plan_model_config = get_plan_model_config()
logger.info(f"plan_model_config:{_redact_model_config(plan_model_config)}\n")
llm_for_plan = set_model(plan_model_config)

act_model_config = get_act_model_config()
logger.info(f"act_model_config:{_redact_model_config(act_model_config)}\n")
llm_for_act = set_model(act_model_config)

tool_model_config = get_tool_model_config()
logger.info(f"tool_model_config:{_redact_model_config(tool_model_config)}\n")
llm_for_tool = set_model(tool_model_config)

vision_model_config = get_vision_model_config()
logger.info(f"vision_model_config:{_redact_model_config(vision_model_config)}\n")
llm_for_vision = set_model(vision_model_config)

credibility_model_config = get_credibility_model_config()
logger.info(f"credibility_model_config:{_redact_model_config(credibility_model_config)}\n")
