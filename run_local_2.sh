...
#SBATCH --job-name=cog-research-miroeval
#SBATCH --output=/home/export/base/ycsc_chenkh/hitici_07/online1/Cog-Research/logs/miroeval_%j.out
#SBATCH --error=/home/export/base/ycsc_chenkh/hitici_07/online1/Cog-Research/logs/miroeval_%j.out
...
REQUESTED_VLLM_PORT="${VLLM_PORT:-8013}"
...
export MIROEVAL_SPLIT="${MIROEVAL_SPLIT:-all}"
export MIROEVAL_TEXT_TASKS_PATH="${MIROEVAL_TEXT_TASKS_PATH:-${PROJECT_ROOT}/data/mirobench_text.json}"
export MIROEVAL_MULTIMODAL_TASKS_PATH="${MIROEVAL_MULTIMODAL_TASKS_PATH:-${PROJECT_ROOT}/data/mirobench_multimodal.json}"
export MIROEVAL_ATTACHMENT_BASE_DIR="${MIROEVAL_ATTACHMENT_BASE_DIR:-${PROJECT_ROOT}/data}"
export MIROEVAL_START_INDEX="${MIROEVAL_START_INDEX:-${DEEPRESEARCH_START_INDEX:-1}}"
export MIROEVAL_MAX_TASKS="${MIROEVAL_MAX_TASKS:-${DEEPRESEARCH_MAX_TASKS:-0}}"
export MIROEVAL_WORKSPACE_ROOT="${MIROEVAL_WORKSPACE_ROOT:-${DEEPRESEARCH_WORKSPACE_ROOT:-${PROJECT_ROOT}/work_space/miroeval_${RUN_TIMESTAMP}}}"
export MIROEVAL_RESULTS_PATH="${MIROEVAL_RESULTS_PATH:-${DEEPRESEARCH_RESULTS_PATH:-${PROJECT_ROOT}/miroeval_results_${RUN_TIMESTAMP}.jsonl}}"
export MIROEVAL_INCLUDE_RUBRIC_IN_PROMPT="${MIROEVAL_INCLUDE_RUBRIC_IN_PROMPT:-${DEEPRESEARCH_INCLUDE_RUBRIC_IN_PROMPT:-0}}"
export MIROEVAL_STRICT_ATTACHMENTS="${MIROEVAL_STRICT_ATTACHMENTS:-true}"

if [[ -n "${MIROEVAL_TASKS_PATH:-}" ]] || [[ -n "${DEEPRESEARCH_TASKS_PATH:-}" ]] || [[ -n "${QUIZ_FILE_PATH:-}" ]]; then
  export MIROEVAL_TASKS_PATH="${MIROEVAL_TASKS_PATH:-${DEEPRESEARCH_TASKS_PATH:-${QUIZ_FILE_PATH:-}}}"
  export DEEPRESEARCH_TASKS_PATH="${DEEPRESEARCH_TASKS_PATH:-${MIROEVAL_TASKS_PATH}}"
  export QUIZ_FILE_PATH="${MIROEVAL_TASKS_PATH}"
else
  unset MIROEVAL_TASKS_PATH || true
  unset DEEPRESEARCH_TASKS_PATH || true
  unset QUIZ_FILE_PATH || true
fi
...
export DEEPRESEARCH_RESULTS_PATH="${DEEPRESEARCH_RESULTS_PATH:-${MIROEVAL_RESULTS_PATH}}"
export DEEPRESEARCH_INCLUDE_RUBRIC_IN_PROMPT="${DEEPRESEARCH_INCLUDE_RUBRIC_IN_PROMPT:-${MIROEVAL_INCLUDE_RUBRIC_IN_PROMPT}}"
...
export TOOL_CALL_MAX_ARG_CHARS="${TOOL_CALL_MAX_ARG_CHARS:-30000}"
...
export REPORT_MAX_CONTENT_LENGTH="${REPORT_MAX_CONTENT_LENGTH:-120000}"
export REPORT_MAX_FILE_PREVIEW_LENGTH="${REPORT_MAX_FILE_PREVIEW_LENGTH:-12000}"
...
# 8. Run cog-research
# -----------------------------------------------------------------------------
echo "Starting cog-research MiroEval runner..."
echo "  MIROEVAL_SPLIT=${MIROEVAL_SPLIT}"
if [[ -n "${MIROEVAL_TASKS_PATH:-}" ]]; then
  echo "  MIROEVAL_TASKS_PATH=${MIROEVAL_TASKS_PATH}"
fi
echo "  MIROEVAL_TEXT_TASKS_PATH=${MIROEVAL_TEXT_TASKS_PATH}"
echo "  MIROEVAL_MULTIMODAL_TASKS_PATH=${MIROEVAL_MULTIMODAL_TASKS_PATH}"
echo "  MIROEVAL_ATTACHMENT_BASE_DIR=${MIROEVAL_ATTACHMENT_BASE_DIR}"
echo "  MIROEVAL_START_INDEX=${MIROEVAL_START_INDEX}"
echo "  MIROEVAL_MAX_TASKS=${MIROEVAL_MAX_TASKS}"
echo "  MIROEVAL_WORKSPACE_ROOT=${MIROEVAL_WORKSPACE_ROOT}"
echo "  MIROEVAL_RESULTS_PATH=${MIROEVAL_RESULTS_PATH}"
